...
1.  **Analyze User Inputs:**
    ...
    *   Farmer-provided Rainfall Estimate: {{{rainfall}}} mm/year
    {{#if soilReport}}*   Soil Report Data: {{{soilReport}}}{{/if}}
    {{#if history}}*   Treatment History: {{{history}}}{{/if}}
    {{#if photoDataUri}}
...
2.  **Simulate Multi-Source Data Ingestion (via Web Search):**
    ...
    *   **Soil Properties:** Search for typical soil properties (e.g., pH, organic matter) for the given soil type and region. Augment this with any specific data from the user's soil report.
...
3.  **Perform Reasoning and Prediction:**
    ...
    *   **Synthesize Data:** Integrate all the information. For example: "The crop is in the grain-filling stage. The provided photo shows some yellowing. The user's soil report indicates low Nitrogen. Weather has been drier than average. The user applied a specific pesticide two weeks ago."
...
4.  **Provide Actionable Outputs:**
    ...
    *   **Actionable Recommendations:** Provide clear, concise recommendations to improve or secure the predicted yield. These should be based on your analysis of all data, including soil reports and history. Examples: "Consider a top-dressing of 25kg/ha of Urea to address the observed yellowing and low soil N." or "Ensure consistent irrigation for the next 2 weeks as the crop is in a critical water-use stage and rainfall has been below average."
