...
const getFarmInsightsFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const prompt = `You are an expert farm manager AI, named KisanMitra. Your primary role is to synthesize various data points and provide a farmer with a simple, prioritized list of actionable insights for the upcoming week.
      ...
      **Instructions:**
      1.  **Synthesize Data:** Review all available data (weather, disease, market). Identify critical connections. For example, does upcoming rain increase disease risk for the specified crop? Does a high market price suggest prioritizing harvest activities?
      ...
      4.  **Create Actionable Recommendations:** For each insight, create a short, clear title and a simple, actionable recommendation. The recommendation should be a concrete task the farmer can perform. For example, instead of "High humidity", say "Preventive Spraying Recommended".
